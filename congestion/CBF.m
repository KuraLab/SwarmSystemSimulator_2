classdef CBF
    %CBF 衝突回避用CBFによる入力制約を行うクラス
    %   1d2次系を想定したCBFの実施
    %{
    使い方
    1. シミュレータ内の冒頭でインスタンスを一つ作る
    2. 単一の入力ベクトルu_iに対して追加する拘束をaddConstraintsで入れていく．パラメタも必要に応じてsetParametersで変える
    3. apply関数で制約適用後の入力を得る
    注）enableされていないとCBFの効果は発動しない
    %}
    ...
    methods
        ...
        function obj = addConstraints(obj,x_,dotx_)
            ...
            Lfh = dotx_;    % [制約数,1]ベクトル
            Lgh = obj.Tc; % [制約数,2]ベクトル
            gammah = obj.gamma*(x_ + dotx_*obj.Tc - obj.rs);   % [制約数,1]ベクトル
            if obj.devide == true
